"""
画像と動画をGeminiで分析するモジュール

このモジュールは、1枚の画像ファイルと1本の動画ファイルをGeminiに送信し、
その内容を分析・説明してもらいます。
"""
...
import src.input_prompts.input_prompt_for_3d_recognition_2026_02_19_with_image_and_movie as ip
...
def extract_args() -> argparse.Namespace:
    ...
    Returns:
        argparse.Namespace: 解析された引数
            ...
            - movie_path: 入力動画ファイルのパス
            - output_path: 出力テキストファイルのパス
            - max_tokens: 最大出力トークン数
            - temperature: 生成の温度パラメータ（0.0-2.0、低いほど確定的）
            - top_p: 生成のtop_pパラメータ（0.0-1.0、核サンプリング）
    """
    parser = argparse.ArgumentParser(description="画像と動画をGeminiで分析する")
    parser.add_argument("--image_path", required=True, help="入力画像ファイルのパス")
    parser.add_argument("--movie_path", required=True, help="入力動画ファイルのパス")
...
if __name__ == "__main__":
    # コマンドライン引数を取得
    args = extract_args()
    assert os.path.exists(args.image_path), f"画像が存在しません: {args.image_path}"
    assert os.path.exists(args.movie_path), f"動画が存在しません: {args.movie_path}"
    ...
    video = load_video(args.movie_path)
    ...
    # Geminiにリクエストを送信（プロンプト、画像、動画の順）
    response = client.models.generate_content(
        model=utils.gemini_model_name,
        contents=[prompt, image, video],
